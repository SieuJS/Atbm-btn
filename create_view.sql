...
BEGIN 
    ...
    ERR_MSG := '';
    SELECT START_TIME INTO START_ENROLL FROM COURSE_REGIS_TIMELINE ; 
    SELECT END_TIME INTO END_ENROLL FROM COURSE_REGIS_TIMELINE ;
    CUR_DATE_ENROLL := SYS_CONTEXT('USERENV','NLS_DATE_FORMAT');

    IF (START_ENROLL > CUR_DATE_ENROLL) OR  (END_ENROLL < CUR_DATE_ENROLL) THEN
        ERR_MSG := 'OUT OF TIME TO ENROL!!!';
        RAISE TXT ;
    ELSE 
        INSERT INTO GRADE_REPORT (STUDENT_NUMBER , SECTION_IDENTIFIER)
        VALUES (STUDENT_USER, :NEW.SECTION_IDENTIFIER);
    END IF ;
    EXCEPTION 
        WHEN TXT THEN 
